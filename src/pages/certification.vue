      ...
      <div class="certification_intro">
        <div class="intro_base">
          <div class="intro_name">
            <label>姓名</label>
            <input type="text" placeholder="请填写您的姓名" v-model="cert_name" maxlength="13">
          </div>
          <div class="intro_card">
            <label>证件号码</label>
            <input type="text" placeholder="请填写真实的身份证号码" v-model="cert_card" maxlength="18">
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      cert_name: function(val) {
        this.setcertname(val)
      },
      cert_card: function(val) {
        this.setcertcard(val)
      },
    },
    created() {
    ...
    methods: {
      ...
      refer: function() {
        let $this = this;
        let certname = $this.certname
        let certcard = $this.certcard
        $this.$axios.post($this.url + 'api/realname_card', {
            name: certname,
            card: certcard,
